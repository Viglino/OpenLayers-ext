...
*/
/**
 * Class: OpenLayers.Control.LayerButtonBar
 * The LayerButtonBar is a visible control composed of a set off button to switch baseLayer visibility.
 *	Overlays of the same group as the baseLayer are shown with the baseLayer
 *
 * Inherits from:
 *  - <OpenLayers.Control>
 */
OpenLayers.Control.LayerButtonBar = OpenLayers.Class(OpenLayers.Control, 
{
	/**
     * APIMethod: destroy
     */
    ...
    */
	...
	{	OpenLayers.Control.prototype.draw.apply(this, arguments);
		...
		{	var l = this.map.layers[i];
			...
			{	btn = this._addButton(l.id, l.group || l.name);
				/*
				// Show overlays associated with the layer
				if (l.group)
				{	for (var j=0; j<this.map.layers.length; j++) if (this.map.layers[j] != l && this.map.layers[j].group == l.group)
					{	this._addButton(this.map.layers[j].id, this.map.layers[j].name);
					}
				}
				*/
			}
	...
	{	var btn = evt.buttonElement;
		if (btn.layer)
		{	var l = this.map.getLayersBy("id",btn.layer);
			if (l.length) 
			{	l = l.pop();
				if (l.isBaseLayer) this.map.setBaseLayer(l);
				this.setLayer();
			}
		}	
    },
    ...
    */
    ...
    CLASS_NAME: "OpenLayers.Control.LayerButtonBar"
});